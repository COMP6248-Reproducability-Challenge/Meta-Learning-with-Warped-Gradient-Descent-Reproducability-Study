...
class DualUpdater:
    ...
    def backward(self, model, step_fn, **opt_kwargs):
        ...
        print("init_time", time.time() - t_init_step)
...
def warp_on_same_loss(model, criterion, trj, brj, tds, step_fn,
        ...
        for (x, y, state, buffer, x2, y2) in batch:
            ...
            if buffer:
                for p, b in zip(
                    ...
                    opt.state[p] = b
            #print("buffer", time.time() - t_buffer)
            ...
            loss = loss + l2
        t_step_loop = time.time() - t_step_loop
        loss = loss / bsz

        t_back_buff = time.time()
        backward(loss, model.meta_parameters(include_init=False))
        t_back = time.time() - t_back_buff

        t_step_buff = time.time()
        step_fn()
        t_ins_steps = time.time() - t_step_buff
        return t_back, t_ins_steps, t_step_loop
    ...
    for _ in range(epochs):
        t_datapoints = time.time()
        #datapoints = [_get(t, i) for t in trj for i in range(len(trj[t]))]

        with joblib.Parallel(n_jobs=-1, backend='threading') as parallel:
            datapoints = parallel(
                joblib.delayed(_get)(t, i)
                for t in trj
                for i in range(len(trj[t]))
            )

        #for x in datapoints:
            #print(x)
        print(len(datapoints))
        print("datapoints", time.time() - t_datapoints)
        np.random.shuffle(datapoints)

        if bsz > 0:
            for i in range(0, len(datapoints), bsz):
                b, s, ss = _step(datapoints[i:i+bsz])
                #t_back_tot += b
                #t_ins_steps_tot += s
                #t_step_tot += ss
        else:
            b, s, ss = _step(datapoints)
            #t_back_tot += b
            #t_ins_steps_tot += s
            #t_step_tot += ss


    print("loop", time.time() - t_loop)
    #print("steps", time.time() - t_steps)
    #print("inside back", t_back_tot)
    #print("inside step", t_ins_steps_tot)
    #print("inside inside step", t_step_tot)
